...
FRAMES = 2048
BLOCK_LENGTHS = [8, 16, 32, 64]            # the DSP runs: N codes 0..3
...
MARKER = 0x01C300
RECORD_WORDS = 3
HEADER_WORDS = CONFIG_WORDS + 2 + 2       # constants, epw and ras, block length and count
FASTEST = 127
...
def segments_text(scenario: Scenario) -> str:
...
def payload(tables: Tables, run: ControlRun, controls: list[list[int]], block: int) -> list[int]:
    config = scenario_config(run.scenario, controls)
    words = config_words(tables, Run(config, run.kernel))
    blocks = FRAMES // block
    words += [run.scenario.epw, run.scenario.ras, block, blocks]
    for b in range(blocks):
        words += controls[b * block]
    assert len(words) == HEADER_WORDS + RECORD_WORDS * blocks
    for word in words:
...
def emit_tables(tables: Tables, oracle: Path) -> str:
    lines = [
        ...
        ";",
        "; Payloads for MT32_CMD_CONTROL_RUN, one per run and block length: the",
        "; kernel's static constants, the pulse-width and resonance terms the",
        "; per-block derivation needs, the block length and count, then one",
        "; record of amp >> 10, pitch and cutoff >> 3 per block. The table at",
        "; the end holds (pointer, word count) per run * 4 + block-length code.",
        "",
    ]
    table = []
    for r, run in enumerate(RUNS):
        ...
        for n, block in enumerate(BLOCK_LENGTHS):
            words = payload(tables, run, controls, block)
            label = f"ctrl_run{r}_n{n}"
            table.append((label, len(words)))
            lines.append(f"{label}:        ; {run.name}, blocks of {block}")
...
def main() -> None:
    ...
    for name, doc in (
        ("segments", "print the scenario's ramp segments for the oracle"),
        ("oracle-args", "print the oracle command line for the run's held render at --block"),
