...
STAGE2_MAGIC = 0x4D544C  # "MTL"
STAGE2_REPLY_OK = 0x4C4F41  # "LOA"
LOADER_FIRST = 0x0040
LOADER_LIMIT = 0x0080
BOOT_LIMIT = 512
...
def emit_include(
    ...
    lines = [
        "; Generated by tools/generate_dsp_stage2.py; do not edit.",
        f"{upper}_BOOT_WORDS equ {len(boot_words)}",
        f"{upper}_STAGE2_TRANSFER_WORDS equ {len(stream)}",
        f"{upper}_STAGE2_SECTION_COUNT equ {section_count}",
        f"{upper}_STAGE2_PROGRAM_WORDS equ {initialized_words}",
        f"{upper}_STAGE2_REPLY_OK equ ${STAGE2_REPLY_OK:06x}",
        "",
        f"{prefix}_bootstrap_image:",
    ]
    lines.extend(format_values("dc.b", boot_bytes, 2, 12))
    lines.extend(["        even", "", f"{prefix}_program_image:"])
...
def main() -> None:
    ...
    parser.add_argument(
        "--prefix",
        default="dsp",
        help="symbol prefix for the emitted image (default: dsp), so a "
        "program can embed more than one",
    )
